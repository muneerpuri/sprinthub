...
// Reusable Priority Bar Chart Component
  ...
  return (
    <Paper variant="outlined" sx={{ p: 3, height: '100%' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <Typography variant="h6" fontWeight="bold">Tasks by Priority</Typography>
      </Box>
      ...
      {priorities.map(p => (
        <Box key={p.label} sx={{ mb: 2 }}>
          ...
          <Box sx={{ height: 8, bgcolor: 'divider', borderRadius: 4, overflow: 'hidden' }}>
            <Box sx={{ height: '100%', width: `${(p.value / total) * 100}%`, bgcolor: p.color, borderRadius: 4 }} />
          </Box>
        </Box>
      ))}
    </Paper>
...
export default function DashboardPage() {
  ...
  const statCards = [
    { title: "Total Tasks", value: totalTasks, icon: <TotalTasksIcon />, color: theme.palette.primary.main },
    { title: "Completed", value: completedTasks, icon: <CompletedIcon />, color: theme.palette.success.main },
    { title: "Pending / In Progress", value: pendingTasks, icon: <PendingIcon />, color: theme.palette.warning.main },
    { title: "High Priority", value: highPriorityCount, icon: <HighPriorityIcon />, color: theme.palette.error.main },
  ];
  ...
  return (
    <DashboardLayout>
      ...
      <Box sx={{ mb: 4 }}>
        ...
        <Typography variant="body2" color="text.secondary">Welcome back! Here's a snapshot of your current workload.</Typography>
      </Box>
