...
import {
    ...
    Chip, Avatar, Grid, Card, CardContent, LinearProgress
} from "@mui/material";
...
export default function ProjectDetailsPage() {
    ...
    // --------------------------------------------------------
    // PROJECT UPDATE / DELETE LOGIC
    // --------------------------------------------------------
    const handleOpenEdit = () => {
        // Use optional chaining to prevent crashes if project isn't fully loaded
        setFormData({
            id: project?.id,
            name: project?.name || "",
    ...
    const groupedTasks = {
        PENDING: projectTasks.filter(t => (t.status || "PENDING").toUpperCase() === "PENDING"),
        IN_PROGRESS: projectTasks.filter(t => (t.status || "").toUpperCase() === "IN_PROGRESS"),
        COMPLETED: projectTasks.filter(t => (t.status || "").toUpperCase() === "COMPLETED"),
    };
    ...
    const handleInvite = async (e) => {
        ...
        try {
            await inviteUser({ projectId, email: inviteEmail, role: inviteRole }).unwrap();
    ...
    // RENDER
    // --------------------------------------------------------
    if (isProjectLoading) return <DashboardLayout><Box sx={{ display: "flex", justifyContent: "center", alignItems: "center", height: "50vh" }}><CircularProgress /></Box></DashboardLayout>;
    if (!project) return <DashboardLayout><Typography>Project not found.</Typography></DashboardLayout>;
    ...
    return (
        <DashboardLayout>
            ...
            {/* Top Color Banner - Added ? to project.color */}
            <Box sx={{ height: 8, width: "100%", bgcolor: project?.color || "#3b82f6", borderRadius: "4px 4px 0 0", mb: 2 }} />

            {/* Header: Title, Badges, and Action Buttons */}
            <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 3 }}>
                <Box>
                    <Typography variant="h4" fontWeight="bold">{project?.name || "Project"}</Typography>
                ...
                <Box sx={{ display: "flex", gap: 1 }}>
                    ...
                    {canDeleteProject && (
                        <Button variant="outlined" color="error" startIcon={<DeleteIcon />} onClick={handleDeleteProject}>
                            Delete
                        </Button>
                    )}
                </Box>
                ...
                <Grid item="true" size={{ xs: 12, md: 5 }}>
                    <Box sx={{ p: 2, bgcolor: "action.hover", borderRadius: 2, border: "1px solid", borderColor: "divider" }}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, mt: 1 }}>
                            {/* Added ? to project.color here too */}
                            <Avatar sx={{ bgcolor: project?.color || "primary.main", width: 40, height: 40, fontSize: "0.9rem", fontWeight: "bold" }}>
                ...
                <Box>
                    ...
                    {totalTasks > 0 && (
                        <LinearProgress
                            ...
                            sx={{ mb: 3, height: 6, borderRadius: 3, bgcolor: "action.hover", "& .MuiLinearProgress-bar": { bgcolor: project?.color || "primary.main" } }}
                        />
                    )}
                    ...
                    {tasksLoading ? (
                        ...
                        <Box sx={{ display: "flex", flexDirection: "column", gap: 1.5 }}>
                            {projectTasks.map((task) => {
                                ...
                                const statusColor = (task.status || "PENDING").toUpperCase() === "COMPLETED"
                                    ? "success"
                                    : (task.status || "").toUpperCase() === "IN_PROGRESS"
                                        ? "info"
                                        : "default";

                                const statusText = (task.status || "PENDING").toUpperCase() === "COMPLETED"
                                    ? "Done"
                                    : (task.status || "").toUpperCase() === "IN_PROGRESS"
                                        ? "In Progress"
                                        : "To Do";
                                ...
                                return (
                                    <Card key={task.id} variant="outlined" sx={{ borderRadius: 1.5, border: "1px solid", borderColor: "divider", bgcolor: "background.paper" }}>
                                        <CardContent sx={{ p: 2, "&:last-child": { pb: 2 }, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
                                            ...
                                            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, flexWrap: "wrap" }}>
                                                <Chip
                                                    ...
                                                    color={statusColor}
                                                    variant="outlined"
                                                    sx={{ height: 22, fontSize: "0.7rem", fontWeight: "medium" }}
                                                />
                                                ...
                                                {task.priority && (
                                                    <Chip
                                                        ...
                                                        sx={{ height: 22, fontSize: "0.7rem", fontWeight: "bold", textTransform: "uppercase" }}
                                                    />
                                                )}
                                                ...
                                                {formattedDate && (
                                                    <Box sx={{ display: "flex", alignItems: "center", gap: 0.5, color: "text.secondary" }}>
                                                        ...
                                                        <Typography variant="caption" sx={{ fontSize: "0.75rem" }}>{formattedDate}</Typography>
                                                    </Box>
                                                )}
                                            </Box>
                                        </CardContent>
                ...
                <Box>
                    {canManageMembers && (
                        <Paper sx={{ p: 3, mb: 4 }} variant="outlined">
                            <Typography variant="h6" mb={2}>Invite User to Project</Typography>
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {isMembersLoading ? (
                                    ...
                                    <TableRow><TableCell colSpan={4} align="center">No members invited yet.</TableCell></TableRow>
                                ) : (
                                    members.map((member) => (
                                        <TableRow key={member.id}>
                                            <TableCell>{member.users?.firstName} {member.users?.lastName}</TableCell>
                                            <TableCell>{member.users?.email}</TableCell>
                                            {member?.userId === project?.ownerId?<TableCell>Owner</TableCell>:<TableCell>
                                                {canManageMembers ? (
                                                    <Select 
                                                        size="small" 
                                                        value={member.role}
                                            ...
                                            {canManageMembers && (
                                                <TableCell align="right">
                                                        ...
                                                        <IconButton color="error" disabled={member?.userId  === project?.ownerId} onClick={() => handleRemoveMember(member.id)}>
                                                            <DeleteIcon />
                                                        </IconButton>
                                                </TableCell>
                                            )}
