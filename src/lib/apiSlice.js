...
export const apiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getTasks: builder.query({
      queryFn: async () => {
        const { data, error } = await supabase.from('tasks').select('*').is('deletedAt', null).order('createdAt', { ascending: false });
    ...
    getProjects: builder.query({
      queryFn: async () => {
        const { data, error } = await supabase.from('projects').select('*, owner:ownerId(firstName, lastName, email)').order('createdAt', { ascending: false });
        if (error) return { error };
        return { data };
      },
    ...
    getProjectById: builder.query({
      queryFn: async (id) => {
        const { data, error } = await supabase.from('projects').select('*, owner:ownerId(firstName, lastName, email)').eq('id', id).single();
        if (error) return { error };
        return { data };
      },
    ...
    inviteUserToProject: builder.mutation({
      queryFn: async ({ projectId, email, role }) => {
        ...
        if (error) {
          if (error.code === '23505') return { error: { message: "User is already a member of this project." }};
          return { error };
        }
        return { data };
      },
    ...
    updateMemberRole: builder.mutation({
      queryFn: async ({ memberId, role, projectId }) => {
        const { data, error } = await supabase.from('project_members').update({ role }).eq('id', memberId).select();
        if (error) return { error };
        return { data };
      },
      invalidatesTags: (result, error, { projectId }) => [{ type: 'ProjectMember', id: projectId }],
    ...
    getComments: builder.query({
      queryFn: async (taskId) => {
        const { data, error } = await supabase.from('comments').select('*, owner:ownerId (firstName, lastName, email)').eq('taskId', taskId).order('createdAt', { ascending: true });
        if (error) return { error };
        return { data };
      },
